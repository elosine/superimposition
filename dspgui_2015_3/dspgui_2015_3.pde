...
void draw() {
  ...
  for (int i=0; i<nfx; i++) {
    ...
    //mouseover stroke
    if (mofx[i]==1 || fxatv[i]==1) stroke(clr.get("orange"));
    else noStroke();
  ...
  for (int i=0; i<nfx; i++) {
    ...
    rect(20+100+bw, 20+((bh+10)*i), bw, bh);
    textAlign(CENTER, CENTER);
    fill(0);
    textFont(f1);
    text( fxlbl[i], (bw/2)+20+100+bw, (bh/2)+((bh+10)*i)+20 );
  }
...
int wfxR() {
  int n = -1;
  for (int i=0; i<nfx; i++) {
    if ( mouseX>(120+bw) && mouseX<(120+bw+bw) && 
      mouseY>20+((bh+10)*i) && mouseY<(20+((bh+10)*i)+bh) ) {
      n=i;
    }
  }
