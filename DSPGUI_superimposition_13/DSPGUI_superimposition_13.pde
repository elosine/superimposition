...
int nsamps = 3; //number of sample banks per side
  ...
  for (int i=0; i<(nsamps*2); i++) {
    if (i<nsamps) {
      for (int j=0; j<nsampfx; j++) {
        pressz.mk(ixsmpfx, margin+((hgap2+bw2)*j), fxsh+(btby2*i)+bh1, bw2, bh2, clrset[j+8], "");
        ixsmpfx = ixsmpfx+1;
      }
    } //
    else {
      int itmp = i-nsamps;
      for (int j=0; j<nsampfx; j++) {
        pressz.mk(ixsmpfx, margin+((hgap2+bw2)*j)+bw1+hgap1, fxsh+(btby2*itmp)+bh1, bw2, bh2, clrset[j+8], "");
        ixsmpfx = ixsmpfx+1;
      }
    }
  }
              ...
              if (inst.ix==sampix2) { // if the index of the button equals the current fxtogL
                if (int(sval)==1) {
                  ...
                  if (inst.on2 == 1) {
                    ...
                    renderWaveform(bufA[samprectog], samprectog); //render waveform display to offscreen buffer
                    imgA[samprectog] = bufA[samprectog].get(0, 0, bufA[samprectog].width, bufA[samprectog].height);
                  }
                } // end if (int(sval)==1)
...
void renderWaveform(PGraphics buffer, int sampnum) {
  osc.send("/getidx", new Object[]{sampnum}, sc); //update waveform array from sc
  buffer.beginDraw();
  buffer.smooth();
  buffer.noFill();
