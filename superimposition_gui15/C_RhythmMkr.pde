...
class RhythmMkr {
  ...
  int[]artnums = new int[0];
  float[] artys = new float[0];
  float[]artxs = new float[0];
  String[]artnames = new String[0];
  ...
  void mkarticul(int numart, int numsusart, String artset, String sartset) {
    rmarts();//first remove existing articulations for this rhythm
    int[]suses = new int[0];
    ...
    int[] ixstmp = new int[dset.length];//make set of available index numbers
    for (int i=0; i<ixstmp.length; i++) ixstmp[i]=i;
    ...
    for (int i=0; i<ixstmp.length; i++) {
      for (int j=0; j<suses.length; j++) {
        if (ixstmp[i]==suses[j])//non sustaining articulations should not be placed on sustaining partials
          if (ixstmp.length>0)  ixstmp = rmArrayIx_int(ixstmp, i);
      }
    }
    //Make Non Sustaining Articulations
    //Check that there are not more articulations than partials
    if (numart>ixstmp.length)numart=ixstmp.length;
    for (int i=0; i<numart; i++) {
      //Only make articulations if there are rhythms available
      if (ixstmp.length>0) {
        int ixxxtmp = floor( random(ixstmp.length) );
        int ixxtmp = ixstmp[ixxxtmp]; //choose a partial to place an articulation
        // println(ixxtmp);
        ...
        if (dset.length>0 && pitch.length==dset.length) { //only make if there are rhythms and pitches
          //Make an index number based on the rhythm number
          int ixtmp = (1000*(ix+1))+i;
          ...
          artnums = append(artnums, ixtmp);//Add svg index numbers to local set to remove later
          artys = append(artys, arty); //set of relative y for clone sets
          artxs = append(artxs, dset[ixxtmp]); //set of relative y for clone sets
          artnames = append(artnames, art); //set of relative y for clone sets
          ixstmp = rmArrayIx_int(ixstmp, ixxxtmp); //take chosen partial out of play so there is no multiple articulations on single partial
        }
      }
    }
    ...
    for (int i=0; i<numsusart; i++) {
      ...
      if (suses.length>0) {
        ...
        if (dset.length>0 && pitch.length==dset.length) { //only make if there are rhythms and pitches
          ...
          artxs = append(artxs, dset[ixxtmp]); //set of relative y for clone sets
          artnames = append(artnames, art); //set of relative y for clone sets
          suses = rmArrayIx_int(suses, ixxxtmp); //take chosen partial out of play so there is no multiple articulations on single partial
        }
      }
    }
  }
