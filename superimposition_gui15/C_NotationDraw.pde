...
class NotationDraw {
  ...
  NotationDraw(int aix, int aogix, int atr) {
    ...
    for (RhythmMkr inst : setORhythmMkr.cset) {
      if (inst.ix == ogix) {
        //make new articulations
        for (int i=0; i<inst.artnums.length; i++) {
            int newartnum = inst.artnums[i]+200;
            setSVGdisplay.mk( newartnum, inst.artnames[i], inst.artxs[i], inst.artys[i]+t0 ); //make articulation
            artnums = append(artnums, newartnum);
        }
